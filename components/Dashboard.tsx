...
import { TrendingUp, AlertCircle, ShoppingBag, DollarSign, Activity } from 'lucide-react';
import { GlassCard } from './GlassCard';
import { StatCardProps, SaleRecord } from '../types';
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <GlassCard className="lg:col-span-2 p-6 min-h-[400px]">
          <div className="flex justify-between items-center mb-6">
            ...
            <select className="bg-white/5 border border-white/10 text-sm text-slate-300 rounded-lg px-3 py-1 outline-none focus:border-primary">
        ...
        <GlassCard className="p-6">
          <div className="flex justify-between items-center mb-6">
            ...
            <button className="text-sm text-primary hover:text-blue-300 transition-colors">View All</button>
          </div>
          <div className="space-y-4">
            {RECENT_SALES.map((sale) => (
              <div key={sale.id} className="flex items-center justify-between p-3 rounded-xl hover:bg-white/5 transition-colors cursor-pointer group">
                ...
                <div className="text-right">
                  ...
                  <span className={`text-[10px] px-2 py-0.5 rounded-full ${sale.status === 'Completed' ? 'bg-green-500/20 text-green-400' : 'bg-yellow-500/20 text-yellow-400'}`}>
                    {sale.status}
                  </span>
                </div>
              </div>
